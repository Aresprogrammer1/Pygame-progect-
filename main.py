...
class Window:
    ...
    def __init__(self):
        ...
        self.con = sqlite3.connect('game.sqlite')
...
class Window1:
    ...
    def __init__(self):
        ...
        self.difficulty = pygame_gui.elements.ui_drop_down_menu.UIDropDownMenu(
            options_list=['Не выбрано', 'Легко', 'Средне', 'Сложно'],
            starting_option='Не выбрано',
            relative_rect=pygame.Rect((225, 150), (200, 50)),
            manager=gui_manager
        )
...
class Game:
    ...
    def __init__(self, labyrinth, hero, enemy):
        ...
        self.con = sqlite3.connect('game.sqlite')
        self.cur = self.con.cursor()
        r_a = self.cur.execute("""SELECT all FROM play WHERE user_id == {}""".format(result)).fetchone()
        self.cur.execute("""INSERT INTO play(all) VALUES('{}') """.format(r_a[0] + 1))
    ...
    while running:
        ...
        if not is_start:
            for event in pygame.event.get():
                ...
                if event.type == pygame_gui.UI_TEXT_ENTRY_FINISHED:
                    ...
                    if int(len(password_result)) != 0 and int(len(log_result)) != 0:
                        reg_result = w.cur.execute(
                            """SELECT user_id FROM user WHERE login = {} and password = {}"""
                            .format(log_result, password_result)).fetchone()
                        result = reg_result
                    if len(result) == 1:
                        result = result[0]
                        log_result = ''
                        ...
                        w.cur.execute(
                            ...
                            .format(log_result, password_result))
                        w.cur.execute("""INSERT INTO play(win, defeat, all) VALUES('{}', '{}', '{}')"""
                                      .format(0, 0, 0))
                        password_result = ''
                        log_result = ''
                        w.log.hide()
        ...
        if is_start:
            ...
            if game.check_win():
                ...
                r_w = w.cur.execute("""SELECT win FROM play WHERE user_id == {}""".format(result)).fetchone()
                w.cur.execute("""INSERT INTO play(win) VALUES('{}') """.format(r_w[0] + 1))
            ...
            if game.check_lose():
                ...
                pygame.mixer.music.stop()
                r_d = w.cur.execute("""SELECT defeat FROM play WHERE user_id == {}""".format(result)).fetchone()
                w.cur.execute(
